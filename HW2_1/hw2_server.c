...
void Game(int p1,int p2,int num)
{
  ...
  if(ans == 'O')
  {
	send(connfd[p1], "<Server>  win !\n", strlen("<Server>  win !\n"),0);
	send(connfd[p2], "<Server>  lose !\n", strlen("<Server>  lose !\n"),0);
  }
  else if(ans == 'X')
  {
	send(connfd[p2], "<Server>  win !\n", strlen("<Server>  win !\n"),0);
	send(connfd[p1], "<Server>  lose !\n", strlen("<Server>  lose !\n"),0);
  }
  else if(ans == 'Q')
  {
...
void displayboard(int num)
{
  ...
  {                                
	for(m=0; m<col; m++)
    {
        if(array[num][l][m]=='N')printf(" * ");
		else printf(" %c ", array[num][l][m]);
		//if(m<col-1)                                
			//printf("|");                           
	}
	printf("\n");
